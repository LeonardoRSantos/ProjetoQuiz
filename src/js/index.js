...
const next = () => {
	...
	if (sumWithoutResponse === currentAnswers.length) {
		...
		if (currentQuestion === answers[selectedType].length) {
			...
			hitsSave.innerHTML = `<span class="></span>`
			winPoints.innerHTML = `<span class="></span>`
...
const renderSavedItems = (index = 0) => {
	...
	const items = localStorage.getItem('registeredItems'),
		defaultHtmlItem = '<table class="responsive-table"><thead><tr><th>Nome</th><th class="center-align">Tempo</th><th class="center-align">Remover</th></tr></thead><tbody>'
